    ...
    def set_initial_poses(self, opt: Dict[str, Any]) -> Tuple[torch.Tensor, List[int], List[int]]:
        ...
        if opt.camera.optimize_relative_poses:
            if opt.camera.initial_pose == 'noisy_gt':
                ...
                initial_poses_w2c = self.train_data.get_all_noise_poses(opt).to(self.device)
...
class PoseAndNerfTrainerPerScene(nerf.NerfTrainerPerScene, CommonPoseEvaluation):
    ...
    def build_pose_net(self, opt: Dict[str, Any]):
        ...
        pose_noise_path="{}/pose_noise.npy".format(cam_path)
        np.save(pose_noise_path,initial_poses_w2c.detach().cpu().numpy())
    ...
    @torch.no_grad()
    def extract_mesh_helper(self, opt: Dict[str, Any]):
        pose, pose_GT = self.get_all_training_poses(self.settings)
        if pose.shape[0] > 9:
            # alignment of the trajectory
            _, self.net.sim3_est_to_gt_c2w = self.prealign_w2c_large_camera_systems(self.settings, pose, pose_GT)
            ...
            # alignment of the first cameras
            _, self.net.sim3_est_to_gt_c2w = self.prealign_w2c_small_camera_systems(self.settings, pose, pose_GT)
        super().extract_mesh_helper(opt)
        return

    @torch.no_grad()
    def extract_mesh(self, opt: Dict[str, Any]):
        self.logger.info("extract_mesh start")
        self.net.eval()

        # self.load_snapshot(checkpoint=opt.max_iter)
        self.load_snapshot()

        self.extract_mesh_helper(opt)

        return

    @torch.no_grad()
    def generate_camera_pose(self, opt: Dict[str, Any]):
        self.net.eval()
        ...
        # pose_ref_path="{}/pose_ref.npy".format(cam_path)
        # np.save(pose_ref_path,pose_ref.cpu().numpy())

        # pose_aligned_path = "{}/pose_aligned.npy".format(cam_path)
        # np.save(pose_aligned_path, pose_aligned.cpu().numpy())

        pose_trained_c2w = self.self.net.pose_net.get_c2w_poses()
        pose_trained_c2w_path = "{}/pose_trained_c2w.npy".format(cam_path)
        np.save(pose_trained_c2w_path, pose_trained_c2w.detach().cpu().numpy())
