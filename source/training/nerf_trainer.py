...
class NerfTrainerPerScene(base.PerSceneTrainer):
    ...
    def __init__(self, opt: Dict[str, Any]):
        ...
        if (self.settings.test_metrics_only or 
            ...
            self.settings.render_video_only or
            self.settings.save_pose_only or
            self.settings.extract_mesh_only):
    ...
    def extract_iso_level(self,density, iso_level=32):
        # Density boundaries
    ...
    def extract_mesh_helper(self, opt: Dict[str, Any]):
        ...
        # mesh_path = "{}/mesh".format(opt.output_path)
        # os.makedirs(mesh_path, exist_ok=True)

        # ret = self.net.render_cube_at_specific_pose_and_rays(opt, mode='val', iter=None,N=N,axis_range=axis_range)
        # sigma=ret["sigma"]
        ...
        # sigma_threshold_list=[25.,50.,75.,100.]

        for N in N_list:
            for axis_range in axis_range_list:
                for iso_level in iso_level_list:
                # for sigma_threshold in sigma_threshold_list:
                    ...
                    trimesh.exchange.export.export_mesh(mesh, "{}/model_N{}_axis_range{}_iso_level{}.obj".format(mesh_path,N,axis_range,iso_level))
