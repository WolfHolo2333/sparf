...
class Graph(torch.nn.Module):
    ...
    def render_cube_at_specific_pose_and_rays(self, opt: Dict[str, Any], iter: int,
                                               mode: str = 'train', per_slice: str = False, N: int = 128, axis_range: float = 1.2) -> Dict[str, Any]:
        ret=self.render_cube(opt,mode=mode,iter=iter,N=N,axis_range=axis_range)
        return ret
    ...
    def render_cube(self, opt: Dict[str, Any],iter: int = None, mode: int = None, N: int = 128, axis_range: float = 1.2) -> Dict[str, Any]:
        ...
        ymin, ymax = -axis_range, axis_range  # forward/backward range
        zmin, zmax = -axis_range, axis_range  # up/down range
        ...
        pred_fine=None
        pred_coarse = self.nerf.forward_cube_samples(opt, xyz_, dir_,
        ...
        if opt.nerf.fine_sampling and compute_fine_sampling:
            pred_fine = self.nerf.forward_cube_samples(opt, xyz_, dir_,
                                                         embedder_pts=self.embedder_pts,
                                                         embedder_view=self.embedder_view, mode=mode)
